...
";
        ...
        public override void ExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                bool isFreshInit = (userPresets.Count == 0);

                if (isFreshInit)
                {
                    // 如果库是空的，我们会填充内置预设
                    // 这些内置预设已经是新标准 (0.1 - 1.0) 了
                    InitLibrary();
                ...
                // 只有当不是全新初始化（即读取了旧存档/旧配置），且从未迁移过时，才执行除法
                if (!_chattinessMigratedV2)
                {
                    MigrateChattinessValues();
                    _chattinessMigratedV2 = true;
                }
            }
        ...
        private void MigrateChattinessValues()
        {
